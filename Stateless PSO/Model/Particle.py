...
class Particle:
    ...
    def SubmitFitnessFunctionJobRequest(self):
        noRetries = 0
        waiting = True
        while waiting:
            try:
                requests.post(self.FitFunUrl, data=self.toJson(), headers={'content-type': 'application/json'})
                waiting = False
            except:
                if noRetries > 100:
                    # If that happens whole algorithm will hangs (PSO loop will not get result)
    ...
    @classmethod
    def GenerateInitial(cls, particleId, noVariable: int, minVal: Vector, maxVal: Vector, fitFunUrl: str) -> 'Particle':
        x = []
        v = []
        ...
        return Particle(particleId, x, v, x, -sys.float_info.max, fitFunUrl)